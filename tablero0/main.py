...
def generar_comida(num_comida):
    ...
    for _ in range(num_comida):
        ...
        while matriz[x][y] != '·':
            x, y = randint(0, len(matriz)-1), randint(0, len(matriz)-1)
        # la cantidad de comida es aleatoria, como str
        matriz[x][y] = str(randint(1, 9))
...
def quien_comienza():
    ...
    # imprime la letra del premer jugador
...
def localiza(jugador):
    ...
    for y in range(len(matriz)):                # y da la posición vertical (son las filas)
        # x da la posición horizontal (son las columnas)
...
def posibilidades(jugador):
    ...
    # [Up, Down, Left, Right] las 4 direcciones en ese orden
    arr_posibilidades = [True] * 4
    # Los 'False' pueden ser por dos motivos:
    # Motivo 1. nos salimos del tablero
    if y == 0:
        arr_posibilidades[0] = False   # UP
    if y == DIM-1:
        arr_posibilidades[1] = False   # DOWN
    if x == 0:
        arr_posibilidades[2] = False   # LEFT
    if x == DIM-1:
        arr_posibilidades[3] = False   # RIGHT
    # Motivo 2. la celda de destino ya está ocupada por otro jugador
    if y > 0 and matriz[x][y-1] in LETRAS:
        arr_posibilidades[0] = False   # UP
    if y < DIM-1 and matriz[x][y+1] in LETRAS:
        arr_posibilidades[1] = False   # DOWN
    if x > 0 and matriz[x-1][y] in LETRAS:
        arr_posibilidades[2] = False   # LEFT
    if x < DIM-1 and matriz[x+1][y] in LETRAS:
        arr_posibilidades[3] = False   # RIGHT
...
def elegir_mover(jugador, arr_posibilidades):
    ...
    x, y = localiza(jugador)
    print(
        f"{LETRAS[jugador]} está en (x,y)=({x},{y}) y su arr_posibilidades es {arr_posibilidades}")
    mi_max = 0  # si la celda contigua es numérica es que hay comida y buscamos su máximo
    if arr_posibilidades[0] and matriz[x][y-1].isnumeric():
        mi_max = max(mi_max, eval(matriz[x][y-1]))
        return 0  # UP
    if arr_posibilidades[1] and matriz[x][y+1].isnumeric():
        mi_max = max(mi_max, eval(matriz[x][y+1]))
        return 1  # DOWN
    if arr_posibilidades[2] and matriz[x-1][y].isnumeric():
        mi_max = max(mi_max, eval(matriz[x-1][y]))
        return 2  # LEFT
    if arr_posibilidades[3] and matriz[x+1][y].isnumeric():
        mi_max = max(mi_max, eval(matriz[x+1][y]))
        return 3  # RIGHT
...
def jugando():
    ...
    while contar_comida() > 0:
        ...
        for i in range(NUM_JUGADORES):
            ...
            # si el array es [False, False, False, False]
            if not any(arr_posibilidades):
                print(
                    f"El jugador {LETRAS[jugador]} está bloquedao y pasa al siguiente")
                continue
            # elegimos en que dirección movernos
            direccion_index = elegir_mover(jugador, arr_posibilidades)
            # ahora nos movemos según la direccion_index: 0:Up 1:Down 2:Left 3:Rigth
            mover(jugador, direccion_index)
