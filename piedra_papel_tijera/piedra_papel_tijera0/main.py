...
def juegoPPT(nums = 0, rein = False):
    ...
    if (int(nums) > 1 and rein == False):
        print('Basado en el numero de partidas a ganar, el total a jugar seran %s partidas' % partidas)
        print('')
    ganador = [0, 0]
    finalizo = False
        ...
        if opcion not in opciones:
            ...
            if (opcion_pc == opcion_us):
                ...
                if (int(nums) in ganador):
                    ...
                    finalizo = True
                    break
                elif (partidas == int(nums) and sum(ganador) == 0):
                    break
        partidas -= 1
    if (finalizo != True):
        print('Partidas terminadas sin ganador o es posible que no haya algun ganador en las siguientes')
